LET ME KNOW IF SOMETHING IS WRONG PLEASE
# hackpad
my contribution to hackclubs blueprint!
I found this project litellary on instagram at the end of its deadline, i have in a next project already started for blueprint.
In this project i will fix the code as i can already feel it wont work as i intended.
ssd1306 oled display fits above xiao but it doesnt cover it, the xiao board is fully visible.
BOM:
*pla or abs shell (3d files in cad folder)
*4x cherry mx keys 
*pcb (files in pcb folder)
*seeed XIAO rp2040-DIP
*4x sk6812 LED
*adafruit ssd1306 oled
*a lot of love
schematic 
<img width="1181" height="742" alt="image" src="https://github.com/user-attachments/assets/7728e22c-8db4-4d22-ad07-3998413268e3" />
``````````````````````````````````````````
pcb
<img width="1038" height="714" alt="image" src="https://github.com/user-attachments/assets/7a1a5e4e-2306-4812-a169-231454f1d3a3" />
``````````````````````````````````````````
case assembled
<img width="852" height="542" alt="image" src="https://github.com/user-attachments/assets/0fe8b0e0-ce2d-4083-971f-8bf0db9b22b9" />
``````````````````````````````````````````
top cover
<img width="790" height="520" alt="image" src="https://github.com/user-attachments/assets/88cff94c-fa7c-46e8-b115-72b455754a87" />
``````````````````````````````````````````
bottom shell
<img width="663" height="520" alt="image" src="https://github.com/user-attachments/assets/46497d38-6e9f-4425-8ed7-613ae1602bbd" />
``````````````````````````````````````````
pcb in 3d
<img width="567" height="488" alt="image" src="https://github.com/user-attachments/assets/85249135-9a67-405a-b907-dc87c4cfba85" />
`````````````````````````````````````````
ssd1306
<img width="302" height="79" alt="image" src="https://github.com/user-attachments/assets/b58397be-e79d-4379-a19c-5a6931f9ff2c" />
`````````````````````````````````````````




